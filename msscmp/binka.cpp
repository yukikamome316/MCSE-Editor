...
MSSCMP_API int wav2binka(wchar_t *wav, wchar_t *binka) {
  ...
  if (ret != 0) {
    printf("wav2bink: |   failed to convert %ls to %ls\n", wav, binka);
    return -1;
  }
  return 0;
}
...
MSSCMP_API int binka2wav(wchar_t *binka, wchar_t *wav) {
  ...
  if (!fs.is_open()) {
    printf("bink2wav: |   failed to open %ls\n", binka);
    return -1;
  }
  ...
  fs.close();
  if (data == nullptr) {
    printf("bink2wav: |   failed to read %ls\n", binka);
    return -1;
  }
  ...
  if (mss32 == nullptr) {
    printf("bink2wav: |   failed to load mss32.dll\n");
    return 1;
  }

  auto AIL_set_redist_directory =
  ...
  if (AIL_decompress_ASI(data, (uint32_t)data_size, ".binka", &converted, &num,
                         0U) == 0) {
    printf("bink2wav: |   failed to decompress %ls\n", binka);
    return 1;
  }
  ...
  fs2.close();
  delete[] converted;
  std::std::filesystem::remove("./mss32.dll");
