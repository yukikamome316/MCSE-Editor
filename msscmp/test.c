...
void basicTest() {
  ...
  checkRet("replace ",
           // msscmp %1  ->   %2
           replaceEntryMsscmp(L"Minecraft/UI/scroll/_914_9764864.binka",
                              L"di.binka"));
  remove("out.msscmp");
  checkRet("save    ",
           // save to %1
...
void binkaTest() {
  checkRet("wav2bink",
           ...
           wav2binka(L"datas/audio/di.wav", L"datas/audio/do.binka"));
  checkRet("bink2wav",
           // convert %1(binka) => %2(wav)
           binka2wav(L"datas/audio/di.binka", L"datas/audio/do.wav"));
}
...
int main(int argc, char const *argv[]) {
  ...
  // size_t converted;
  // ret = mbstowcs_s(&converted, msscmpPath, strlen(argv[1]) * 2, argv[1],
  // strlen(argv[1]) * 2); if (ret != 0)
  //{
  //    char error[256];
